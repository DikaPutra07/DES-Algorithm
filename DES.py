...
if __name__ == "__main__":
	...
	for bit in key:
		...
		temp += bit
		if count%4 == 0:
			print(f"{key_hex[count_k]} = {temp}")
			temp.clear()
			count_k+=1
	print(f"binary dari key = {key}\n")
	...
	print("Round Key (56-bit) : ", bin2hex(key))
	print("Split Binary Round Key : ", left, " ", right)
	...
	for i in range(0, 16):
		...
		print("left shift ke-", i+1, " : ", left, " ", right)
		...
		print("->", bin2hex(round_key))

		rkb.append(round_key)
		rk.append(bin2hex(round_key))

	print("\nEncryption")
	cipher_bin = encrypt(pt, rkb, rk)
	print("\nCipher dalam binary : ", cipher_bin)
	cipher_text = bin2hex(cipher_bin)
	print("\n")
